...
impl TileBox3 {
	pub fn new(min: TileCoord3, max: TileCoord3) -> Self {
		assert!(min.x <= max.x && min.y <= max.y && min.z <= max.z, "Invalid TileBox3 bounds");
		Self { min, max }
	}

	pub fn contains(&self, c: TileCoord3) -> bool {
		c.x >= self.min.x && c.x <= self.max.x &&
		c.y >= self.min.y && c.y <= self.max.y &&
		c.z >= self.min.z && c.z <= self.max.z
	}
	...
	pub fn iter_tiles(&self) -> impl Iterator<Item = TileCoord3> {
		...
		(min.z..=max.z).flat_map(move |z| {
			(min.y..=max.y).flat_map(move |y| {
				(min.x..=max.x).map(move |x| TileCoord3 { x, y, z })
			})
		})
	}
...
mod tests {
	...
	fn border_includes_edges() {
		...
		assert!(border.iter().all(|c| c.x == 0 || c.x == 2 || c.y == 0 || c.y == 2 || c.z == 0));
	}
}
